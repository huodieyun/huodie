	...
	public function start() {
		...
		if(strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
            ...
            if($message['msgtype'] === 'event' && $message['event'] === 'LOCATION'){
                $info = pdo_fetch("SELECT * FROM cm_tg_member WHERE openid='{$message['from']}' and uniacid={$_W['uniacid']}");
                if(!$info['name']){
                    $info['name'] = $info['nickname'];
                }
                $has = pdo_fetch("SELECT * FROM cm_tg_sendhello WHERE openid='{$message['from']}' and uniacid={$_W['uniacid']}");
                if(empty($has)){
                    pdo_insert('tg_sendhello',array('openid'=>$message['from'],'uniacid'=>$_W['uniacid'],'time'=>$message['time'],'name'=>$info['name']));
                }else{
                    pdo_update('tg_sendhello',array('time'=>$message['time'],'name'=>$info['name']),array('id'=>$has['id']));
                }
//                //打招呼 获取后台设置 自动发送消息
//                $setting = pdo_fetch("SELECT * FROM cm_tg_autohello WHERE uniacid={$_W['uniacid']}");
//                if($setting['is_on'] == 1){
//                    //
//                    sleep($setting['is_ignore']);
//                    $openid = $message['from'];
//                    $memberinfo = pdo_fetch("SELECT * FROM cm_tg_member WHERE openid={$openid}");
//
//                    if(!$memberinfo['name']){
//                        $memberinfo['name'] = $memberinfo['nickname'];
//                    }
//
//                    $content = $setting['hello_coment'];
//                    $time = $message['time'];
//                }
            }
	...
	private function analyzeSubscribe(&$message) {
		...
		$mappingfans=pdo_fetch("select * from ".tablename('mc_mapping_fans')." where openid=:openid",array(':openid'=>$_W['openid']));
		if($mappingfans['unfollowtime']==0||empty($mappingfans))
		{
			...
			$newusersetting = uni_setting($_W['uniacid'], array('newuser'));
			if(!empty($newusersetting['newuser'])) {
				$message['content'] = $newusersetting['newuser'];
				$params += $this->analyzeText($message);
			}
		}
