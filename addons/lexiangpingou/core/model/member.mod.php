 ...
 */
 ...
 function member_update_by_params($data=array(),$params='') {
	...
	function checkMember($openid = '') {
        ...
        $s1 = pdo_fetch("SELECT id FROM " . tablename('tg_member') . "where unionid=:unionid and uniacid=:uniacid limit 1",array(':uniacid' => $_W['uniacid'], ':unionid' => $userinfo['unionid']));
        if(empty($s1) && empty($member)){
            $parentid = $parentid;
        }else{
            $parentid = -1;
        }
