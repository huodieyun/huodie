...
function coupon_goodscanuse($openid, $payprice,$goodsid){
	global $_W;
	$where3 = "SELECT * FROM ".tablename('tg_coupon').' WHERE `openid` = :openid AND `use_time` = :use_time AND `start_time` < :now1 AND `end_time` > :now2  and goodsid in(0,:goodsid)  ORDER BY `end_time` DESC ';
	$params3 = array(
		':openid' => $openid,
		':goodsid' => $goodsid,
		':use_time' => 0,
	...
	foreach ($coupon3 as $key3 => $value3) {
		$ps=pdo_fetch('select enable from '.tablename('tg_coupon_template') .' where id=:id',array(':id'=>$value3['coupon_template_id']));
		if($ps['enable']==0)
		{
			unset($coupon3[$key3]);
			continue;
...
function coupon_canuses($openid, $payprice){
	global $_W;
	$where3 = "SELECT * FROM ".tablename('tg_coupon').' WHERE `openid` = :openid AND `use_time` = :use_time AND `start_time` < :now1 AND `end_time` > :now2 and `at_least`<=:at_least and `cash`<:at_least and `is_at_least` =2  ORDER BY `cash` DESC limit 1';
	$params3 = array(
		':openid' => $openid,
		':use_time' => 0,
		':at_least' => $payprice,
...
function coupon_canuse($openid, $payprice){
	global $_W;
	$where3 = "SELECT * FROM ".tablename('tg_coupon').' WHERE `openid` = :openid AND `use_time` = :use_time AND `start_time` < :now1 AND `end_time` > :now2  ORDER BY `cash` DESC ';
	$params3 = array(
		':openid' => $openid,
		':use_time' => 0,
		':now1' => TIMESTAMP,
