 ...
 */
	function order_get_list($args = array()) {
		...
		$buyuniacid = !empty($args['buyuniacid'])? trim($args['buyuniacid']): '';
		if (!empty($buyuniacid)) {
			$condition .= " and buyuniacid ='".$buyuniacid."'";
		}
		
		if ($usepage) {
			$sql = "SELECT * FROM " . tablename('tg_function_order') . " where 1  {$condition} {$orderby} LIMIT " . ($page - 1) * $pagesize . ',' . $pagesize;
		} else {
			$sql = "SELECT * FROM " . tablename('tg_function_order') . " where 1  {$condition} ";
		} 
		
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM '.tablename('tg_function_order')."where 1 $condition ",$params);
 ...
 */
	...
	function order_get_by_params($params = '') {
		...
		$sql = "SELECT * FROM " . tablename('tg_function_order') . " {$params} ";
		$order = pdo_fetch($sql);
		return $order;
	}
/**
 ...
 */
	function order_update_by_params($data,$params) {
		global $_W;
		$flag = pdo_update('tg_function_order',$data,$params);
		return $flag;
	}
/**
 * 函数insertorder，插入新订单
 ...
 */
	function order_insert($data) {
		global $_W;
		$flag = pdo_insert('tg_function_order',$data);
		if($flag){
 ...
 */
	function order_delete($id) {
		global $_W;
		$flag = pdo_delete('tg_function_order',$id);
		return $flag;
	}
	...
	function order_get_by_id($id = '') {
		...
		$sql = "SELECT * FROM " . tablename('tg_function_order') . " where uniacid = {$_W['uniacid']} and id = {$id}";
		$order = pdo_fetch($sql);
		return $order;
	}