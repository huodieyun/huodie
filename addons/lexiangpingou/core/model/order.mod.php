 ...
 */
	function order_get_list($args = array()) {
		...
		$g_id = !empty($args['g_id'])? trim($args['g_id']): '';
		if (!empty($g_id)) {
			$condition .= " and g_id ='".$g_id."'";
		}
		...
		foreach($list as $key=>&$value){
			...
			$ord=pdo_fetch('select * from '.tablename('tg_order_level_refund').' where status=2 and orderno=:orderno',array(':orderno'=>$value['orderno']));
			$value['refund']=round($ord['refundprice'],2);
			$value['realprice']=round($value['pay_price'],2)-round($ord['refundprice'],2);
		}
		$data = array();
		$data['list'] = $list;
		$data['total'] = $total;
