...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		$pay = new WxPayConfig();
        $pay_config = $pay->getConfig();
		$inputObj->SetAppid($pay_config['appid']);//公众账号ID
		$inputObj->SetMch_id($pay_config['mchid']);//商户号
	...
	public static function refund($inputObj, $timeOut = 6)
	{
		...
		$xml = $inputObj->ToXml();
		$startTimeStamp = self::getMillisecond();//请求开始时间
        file_put_contents(TG_DATA . "xxx.log", var_export(array('in' => $inputObj , 's'=>$startTimeStamp , 'r'=>(self::postXmlCurl($xml, $url, true, $timeOut)), 't' => date("Y-m-d H:i:s")), true) . PHP_EOL, FILE_APPEND);
        return $inputObj;
