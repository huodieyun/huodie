...
if($do == 'display') {
	...
	include wl_template('gzhset/reply',array('mo'=>$m));
...
if($do == 'post') {
	...
	if(checksubmit('submit')) {
		...
		if (!empty($rid)) {
			...
			echo "<script>alert('回复规则保存成功！');location.href='".web_url('gzhset/reply', array('mo' => $m,'op'=>'post', 'rid' => $rid))."';</script>";
		exit;
		} else {
			message('回复规则保存失败, 请联系网站管理员！');
		}
	}
	include wl_template('gzhset/reply-post',array('mo'=>$m,'op'=>'post'));
...
if($do == 'delete') {
	...
	foreach($rids as $rid) {
		...
		$reply = reply_single($rid);
		if(empty($reply) || $reply['uniacid'] != $_W['uniacid']) {
			$tip='抱歉，您操作的规则不在存或是已经被删除！';
		echo "<script>alert('".$tip."');location.href='".url('platform/reply', array('m' => $m))."';</script>";
			exit;
		}
	...
	echo "<script>alert('规则操作成功！');location.href='".web_url('gzhset/reply', array('mo' => $m))."';</script>";
		exit;
