...
if($op == 'display') {
	...
	include wl_template('gzhset/menu');
...
if($op == 'push') {
	...
	if(is_error($ret)) {
		...
		message('推送成功', web_url('gzhset/menu/display'), 'success');
	}
}
...
if($op == 'copy') {
	...
	if(empty($menu)) {
		message('菜单不存在或已经删除', web_url('gzhset/menu/display'), 'error');
	}
	if($menu['type'] != 3) {
		message('该菜单不能复制', web_url('gzhset/menu/display'), 'error');
	}
	...
	$id = pdo_insertid();
	header('Location:' . web_url('gzhset/menu/add', array('id' => $id)));
	die;
}
...
if($op == 'remove') {
	...
	if ($_GPC['op'] == 'recover') {
		...
		message('恢复菜单成功，是否推送到微信？<a href="'.web_url('gzhset/menu/push', array('id' => $id)).'" class="btn btn-primary">是</a> <a href="'.web_url('gzhset/menu/display').'" class="btn btn-default">取消</a>', web_url('gzhset/menu/display'), 'success');
	}
	$status =  $_GPC['status'];
	if($data['type'] == 1 || ($data['type'] == 3 && $data['menuid'] > 0) && $status != 'history') {
		...
		if(is_error($ret) && empty($_GPC['f'])) {
			$url = web_url('gzhset/menu/remove', array('id' => $id, 'f' => 1));
			$url_display = web_url('gzhset/menu/display', array('id' => $id, 'f' => 1));
	...
	message('删除菜单成功', web_url('gzhset/menu/display'), 'success');
}
