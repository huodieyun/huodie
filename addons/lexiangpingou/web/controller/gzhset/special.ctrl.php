...
//$do = !empty($_GPC['op']) && in_array($do, $dos) ? $do : 'display';
$do=$_GPC['op'];
load()->model('reply');
load()->model('account');
...
if($do == 'display') {
	...
	$setting = uni_setting($_W['uniacid'], array('default', 'welcome','newuser'));
	include wl_template('gzhset/special');
	exit;

}
if($do == 'ckajax'){
	$settings = array(
			'default' => trim($_GPC['dtdefault']),
			'welcome' => trim($_GPC['welcome']),
			'newuser' => trim($_GPC['newuser']),
		);
