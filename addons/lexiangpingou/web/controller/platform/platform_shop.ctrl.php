...
//        die(json_encode($data));
        if (empty($data['id'])) {
            ...
            pdo_insert('tg_platform_shop', $data);
            $tip = '保存成功！请进行二次确认提交并提交审核绑定微信，以方便后续消息接受';
            echo "<script>location.href='" . web_url('platform/platform_shop', array('step' => $step)) . "';</script>";
            exit;
        } else {
            $data['updatetime'] = TIMESTAMP;

            $openid = pdo_fetch("select openid,financial_price from " . tablename('tg_platform_shop') . " where id = " . $data['id']);
            ...
            pdo_update('tg_platform_shop', $data, array('id' => $data['id']));
            if (!empty($openid['openid'])) {
                echo "<script>location.href='" . web_url('platform/platform_shop_list') . "';</script>";
                exit;
            }
            $tip = '提交成功！';
            echo "<script>location.href='" . web_url('platform/platform_shop', array('step' => $step)) . "';</script>";
    ...
    $supplier = pdo_fetch("select * from " . tablename('tg_platform_shop') . " where uniacid = '{$uniacid}' ");
}
if ($op == 'detail') {
    $id = $_GPC['id'];
    $supplier = pdo_fetch("select * from " . tablename('tg_platform_shop') . " where id = '{$id}' ");
...
//        file_put_contents("aa.log", var_export(array('s' => "入口", 'time' => TIMESTAMP), true) . PHP_EOL, FILE_APPEND);
        $smsConf = array(
            ...
            'tpl_id' => '49496', //您申请的短信模板ID，根据实际情况修改
...
include wl_template('platform/platform_shop');
exit();
?>