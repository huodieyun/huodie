...
if ($do == 'list') {
	...
	include wl_template('goods/activity/coupon_template_list');
}
if ($do == 'create' || $do == 'edit') {
	...
	if ($id) {
		...
		if (empty($coupon_template)) {
			$tip='非法访问：访问记录不存在';
		echo "<script>alert('".$tip."');location.href='".url('goods/activity/list')."';</script>";
			exit;
		}
	}
	if (checksubmit('submit')) {
		...
		message($insert ? '添加成功' : '编辑成功', web_url('goods/activity/coupon_template/edit', array('id' => $id)), 'success');
	}
	...
	include wl_template('goods/activity/coupon_template_edit');
}
