...
$allendtime = pdo_fetchall("SELECT update_time,form_id FROM ".tablename('workform')." WHERE status <> 2  ", array());
...
if($op=='display'){
	...
	if ($status == '-11') {
		$condition .= " and status = -1 and vip = 1 ";
	}
	if ($status == '11') {
		$condition .= " and status = 1 and vip = 1 ";
	}
	...
	if ($status == '7') {
		$condition .= " and status <> 2 and status <> -2 and status <> -3";
	}
	if ($status == '2') {
		$condition .= " and status = 2 or status = -2 ";
	}
	if ($status == '-3') {
	...
	$allform = pdo_fetchall("select develop_status,type,title,form_id,status,FROM_UNIXTIME(create_time,'%Y-%m-%d  %H:%i') as create_time,name,vip from" . tablename('workform') .  "  "  .$condition. " and inner_work = 0 order by create_time desc " . "LIMIT " . ($pindex - 1) * $psize . ',' . $psize,array());
	
	$allform2 = pdo_fetchall("select vip,title,form_id,status,FROM_UNIXTIME(create_time,'%Y-%m-%d  %H:%i') as create_time,name from" . tablename('workform') . "  " .$condition. " and inner_work = 0 order by create_time desc",array());
	$total = count($allform2);
	$pager = pagination($total, $pindex, $psize);
	include wl_template('service/workform_list_admin');
}
...
if($op=='replay'){
	...
	echo "<script>location.href='".web_url('service/workform_list_admin/detail',array('form_id' =>$form_id,'after' => 1))."';</script>";
	//echo "<script>location.href='".web_url('service/workform_list_admin',array('op' =>'display','status'=>$_GPC['status']))."';</script>";
	exit();
