...
$abs = '公众号ID';
$unib = '';
$autoorder = pdo_fetchall('select uniacid,orderno from' . tablename('tg_order') . "  WHERE `status`=8 and hexiaoma<>'' and orderno='' ORDER BY uniacid asc");
foreach ($autoorder as $unis => $v) {
    $_W['uniacid'] = $v['uniacid'];
    wl_load()->classs('qrcode');
//    $createqrcode = new creat_qrcode();
//    $createqrcode->creategroupQrcode($v['orderno']);
}
//message(count($autoorder));
//查询自动退款公众号
...
foreach($autouniacid as $unis=>$v){
    ...
    if($ss['auto_refund']==1)
    {
        ...
        $vs.=$v.'<br>';
        $sql = "SELECT orderno,id,openid,price,uniacid FROM " . tablename('tg_order') . " where mobile<>'虚拟' and status=10 and openid<>'' and transid<>''  and uniacid=".$v."  order by ptime asc limit 1";
