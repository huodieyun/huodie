...
if ($op == 'display') {
    ...
    $time = $_GPC['time'];
    //下单时间
    $ztstore = intval($_GPC['comadd']);
    //自提点
    if ($ztstore) {
        $condition .= " and comadd = {$ztstore} ";
    }
...
//    if ($_W['user']['merchant_id'] > 0) {
    ...
    $condition .= ' and supply_goodsid = 0 and dispatchtype = 3';
//    }
...
//die(json_encode($sql));
    ...
    foreach ($list as &$value) {
        ...
        $store = pdo_fetch("select storename from" . tablename('tg_store') . " where id ='" . $value['checkstore'] . "'");
            $realstore = pdo_fetch("select storename from" . tablename('tg_store') . " where id ='" . $value['checkstore'] . "'");
        ...
        if (is_uploaded_file($uploadfile)) {
            ...
            if ($ftype == 'csv') {
                ...
                for ($i = 1; $i < $len_result; $i++) { //循环获取各字段值
                    ...
                    $storename = trim(iconv('gb2312', 'utf-8', $result[$i][23]));
                    $ropenid = trim(iconv('gb2312', 'utf-8', $result[$i][24]));

                    $checkstore = pdo_fetch("select id from" . tablename('tg_store') . " where  uniacid ='{$_W['uniacid']}' and   storename = '" .$storename ."'");
                    $veropenid=pdo_fetch("select openid,storeid from" . tablename('tg_saler') . " where status=1 and  uniacid ='{$_W['uniacid']}'  and  id = '" .$ropenid ."'");
                    $updatezt=false;
                    if(!empty($checkstore['id'])&&!empty($veropenid['openid'])){
                        if($veropenid['storeid']==''){
                            $updatezt=true;
                        }else{
                            $store_id = explode(",", $veropenid['storeid']);
                            foreach($store_id as $val){
                                if($val==$checkstore['id']){
                                    $updatezt=true;
                                }
                            }
                        }
                        ...
                        if ($order['dispatchtype'] == 3) {
                            ...
                            if ($order['is_once_card'] == 1 && (int)$order['once_card_num'] > (int)$order['once_card_ynum'] ) {
                                ...
                                $dat = array(
                                    ...
                                    'veropenid' =>$veropenid['openid'],
                                    'hexiao_man' => pdo_get('tg_member', array('openid' => $veropenid['openid']), array('nickname'))['nickname'],
                                    'mobile' => $order['mobile'],
                                    ...
                                    'comadd' => $order['comadd'],
                                    'checkstore' =>$checkstore['id'],
                                    'ziti_address' => pdo_get('tg_store', array('id' => $checkstore['id']), array('storename'))['storename'],
                                    'uniacid' => $_W['uniacid'],
                                );
                                pdo_insert('tg_order_once_card_record', $dat);
                                $res = pdo_update('tg_order', array('status' => 8,'once_card_ynum'=>$order['once_card_ynum']+1), array('orderno' => $orderNo, 'status' => 8));
                            }else{
                                $res = pdo_update('tg_order', array('status' => 3, 'checkstore' => $checkstore['id'], 'veropenid' => $veropenid['openid'], 'over_time' => TIMESTAMP, 'gettime' => TIMESTAMP,'hexiaotime'=>TIMESTAMP,'is_hexiao'=>2), array('orderno' => $orderNo, 'status' => 8));
                            }
                            if ($res) {
                                $succ_result += 1;
                            } else {
    ...
    message('导入自提订单操作成功！成功' . $succ_result . '条，失败' . $error_result . '条', referer(), 'success');
} elseif ($op == 'output') {
...
//	$orders = pdo_fetchall("select * from" . tablename('tg_order') . "where $condition and status=2");
    ...
    $html = "\xEF\xBB\xBF";
    /* 输出表头 */
    $filter = array('aa' => '订单编号', 'sj' => '所属商家', 'll' => '团ID', 'bb' => '姓名', 'cc' => '电话', 'm2' => '运费', 'dd' => '总价(元)', 'ee' => '状态', 'ff' => '下单时间', 'gg' => '商品名称', 'pp' => '商品规格', 'oo' => '购买数量', 'mm' => '买家留言', 'h1' => '省', 'h2' => '市', 'h3' => '区', 'h4' => '详细地址', 'm6' => '地址类型', 'm7' => '商家备注', 'ii' => '微信订单号', 'm1' => '配送方式', 'mz' => '组团成功时间','md'=>'自提点','st'=>'实际自提点','sa'=>'自提核销员标志id');
    ...
    foreach ($orders as $k => $v) {
        ...
        if ($v['g_id'] > 0) {
            ...
            $comadd = pdo_fetch("select storename from" . tablename('tg_store') . " where uniacid ='{$_W['uniacid']}' and  id = '" .$v['comadd'] ."'");
            $orders[$k]['md']=$comadd['storename'];

            $checkstore = pdo_fetch("select storename from" . tablename('tg_store') . " where uniacid ='{$_W['uniacid']}' and   id = '" .$v['checkstore'] ."'");
            $orders[$k]['st']=$checkstore['storename'];
            $veropenid=pdo_fetch("select id from" . tablename('tg_saler') . " where uniacid ='{$_W['uniacid']}' and  openid = '" .$v['veropenid'] ."'");
            $orders[$k]['sa']=$veropenid['id'];
            foreach ($filter as $key => $title) {
                $html .= $orders[$k][$key] . "\t,";
            }
            $html .= "\n";
...
include wl_template('order/pick_delivery');
exit();
?>