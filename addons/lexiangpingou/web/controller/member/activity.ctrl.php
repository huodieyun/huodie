...
if ($do == 'list') {
    ...
    $total = pdo_fetchall("select id from".tablename('tg_coupon_template')." $where " ,$params);
	$total = count($total);
	...
	include wl_template('member/activity/coupon_template_list');
}
if ($do == 'create' || $do == 'edit') {
	...
	if ($id) {
		...
		if (empty($coupon_template)) {
			$tip='非法访问：访问记录不存在';
		echo "<script>alert('".$tip."');location.href='".url('member/activity/list')."';</script>";
			exit;
		}
	}
	if (checksubmit('submit')) {
		...
		if (empty($id)) {
			...
			pdo_update('tg_coupon_template', $tg_coupon_template_update, array('id' => $id));
		}
		message($insert ? '添加成功' : '编辑成功', web_url('member/activity/coupon_template/edit', array('id' => $id)), 'success');
	}
	...
	include wl_template('member/activity/coupon_template_edit');
}
...
if ($do == 'createQrcode') {
    ...
    for ($i=0;$i<$num;$i++){
        ...
        $qrcode = $qr->createCouponQrcode($content,$template_id);

        $files[$i]['url'] = $qrcode['url'];
        $files[$i]['name'] = $qrcode['name'];
        if(!pdo_insert('tg_coupon_qrcode', $data)){
            message(error(0, '生成二维码失败!'));
        }
    }
    //压缩文件
    $zipurl = $qr->zipCouponQrcode($files,$template_id);
    $data = array(
        'qrcode_url' => $zipurl,
        'have_qrcode' =>1
    );
    pdo_update('tg_coupon_template',$data,['id'=>$template_id]);
