...
$ops = array('update_member','setting','display','modal','credit_record','ajax');
...
if ($op == 'display') {
	...
	foreach($list as &$li){
		$li['pname']='系统管理员';
		if($li['parentid']!=99&&$li['parentid']>0)
		{
			$pmember =pdo_fetch("select realname,nickname from cm_tg_member where uniacid={$li['uniacid']} and id={$li['parentid']}");
			$li['pname']=$pmember['realname']."<br>".$pmember['nickname'];
		}
        unset($li);
...
if($op='update_member'){
	...
	pdo_update('tg_member',array('parentid'=>$parentid),array('id'=>$uid));
