...
$_W['page']['title'] = $checkfunction['name'];	
	...
	$fans=pdo_fetchall("select openid from ".tablename('tg_puv_record')." where uniacid=".$_W['uniacid']." and openid<>'' and createtime>".$stime." GROUP BY openid ORDER BY createtime DESC  ");
	...
	include wl_template('member/notice');
	exit();
}
if($op=='send'){
	...
	if($msgtype == 'text') {
		$send['text'] = array('content' => urlencode($_GPC['txt']));
