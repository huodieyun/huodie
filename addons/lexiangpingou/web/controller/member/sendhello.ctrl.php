...
$ops = array('send','keyword', 'fans', 'transmit', 'post', 'tpl', 'log', 'end');
$op=$_GPC['op'];
$op = in_array($op, $ops) ? $op : 'fans';
$shop = pdo_fetch("SELECT * FROM " . tablename('account_wechats') . " WHERE uniacid = :uniacid", array(':uniacid' => $_W['uniacid']));
if($shop['vip']==0){
    echo "<alert>你无权进行该操作</alert>";
    exit();
}
...
if($op == 'keyword') {
	if($_W['isajax']) {
		...
		$data = pdo_fetchall('SELECT content, module, rid FROM ' . tablename('rule_keyword') . " WHERE uniacid = :uniacid AND status != 0 " . $condition . ' ORDER BY uniacid DESC,displayorder DESC ', array(':uniacid' => $_W['uniacid']));
		$exit_da = array();
		if(!empty($data)) {
			foreach($data as $da) {
...
if($op == 'fans') {
	...
	$stime=TIMESTAMP-86400*2;
	$fans=pdo_fetchall("select openid from ".tablename('tg_sendhello')." where uniacid=".$_W['uniacid'].$condition);
	$fanslist=json_encode($fans);
	$num=count($fans);
	//message($fanslist);
	...
	include wl_template('member/sendhello');
	exit();
}
if($op=='send'){


	$strarr = explode(",",$_GPC['openid']);
	foreach($strarr as $newstr) {
        ...
        $info = pdo_fetch("SELECT * FROM cm_tg_member WHERE openid='{$newstr}' and uniacid={$_W['uniacid']}");
        if(!$info['name']){
            $info['name'] = $info['nickname'];
        }
        $title = $_GPC['title'];
        $title = str_replace('#name#',$info['nickname'],$title);
        ...
        if($msgtype == 'text') {
            $send['text'] = array('content' => urlencode($title.$_GPC['txt']));
        } elseif($msgtype == 'image') {
            $send['image'] = array('media_id' => $_GPC['txt']);
            ...
            $module = $rule['module'];
            $reply = pdo_fetchall("SELECT id ,rid,parent_id,concat('{$title}',title) as title,author,description,thumb,content,url,displayorder,incontent,createtime FROM " . tablename($module . '_reply') . ' WHERE rid = :rid', array(':rid' => $rid));
            if($module == 'cover') {
                $idata['do'] = $reply[0]['do'];
            ...
            if(!empty($reply)) {
                foreach($reply as $c) {
                    ...
                    if(strexists($c['url'], 'http://') || strexists($c['url'], 'https://')) {
                        ...
                        $row['url'] =  "http://".$_W['uniaccount']['key'].".www.lexiangpingou.cn/" .  'app/index.php?i=' .$_W['uniacid'].'&c=entry&id='.$c['id']."&do=detail&m=news" ;
                    }
                    $news[] = $row;
                }
        ...
        if($acid) {
            ...
            if(is_error($data)) {
                internal_log('sendhello',array('status' => 'error', 'message' => $data['message'],'openid'=>$newstr));
...
if($op == 'post') {
	...
	if($msgtype == 'text') {
		...
		$idata = array('rid' => $rid, 'name' => $rule['name'], 'module' => $rule['module']);
		$module = $rule['module'];
		$reply = pdo_fetchall("SELECT id ,rid,parent_id,concat('hahahah',title) as title,author,description,thumb,content,url,displayorder,incontent,createtime FROM " . tablename($module . '_reply') . ' WHERE rid = :rid', array(':rid' => $rid));
		if($module == 'cover') {
			$idata['do'] = $reply[0]['do'];
			$idata['cmodule'] = $reply[0]['module'];
		}
