...
if($op == 'display') {
	...
	if(checksubmit('submit')) {
		if (!empty($_GPC['delete'])) {
			...
			message('粉丝删除成功！', url('member/fans/', array('type' => $_GPC['type'])), 'success');
		}
	}
	$acid = $_W['acid'];
	if($_W['isajax']) {
...
if($op == 'initsync') {
	...
	if(intval($_GPC['page']) == 0) {
		$tip='正在更新粉丝数据,请不要关闭浏览器';
		echo "<script>alert('".$tip."');location.href='".web_url('member/fans/initsync', array('page' => 1, 'acid' => $acid))."';</script>";
			exit;
	...
	if(!empty($ds)) {
		foreach($ds as $row) {
			...
			if(!is_error($fan) && $fan['subscribe'] == 1) {
				...
				$record['tag'] = base64_encode($record['tag']);
				pdo_update('mc_mapping_fans', $record, array('fanid' => $row['fanid']));
                pdo_update('tg_member', array('unionid'=>$fan['unionid'],'nickname'=>$fan['nickname'],'avatar'=>$fan['headimgurl']), array('openid' => $row['openid']));
                if(!empty($row['uid'])) {
					$user = mc_fetch($row['uid'], array('nickname', 'gender', 'residecity', 'resideprovince', 'nationality', 'avatar'));
					$rec = array();
					if(empty($user['nickname']) && !empty($fan['nickname'])) {
	...
	if($pindex > $total_page) {
		$tip='粉丝数据更新完成';
		echo "<script>alert('".$tip."');location.href='".web_url('member/fans')."';</script>";
			exit;
		
	} else {
		$tip='正在更新粉丝数据,请不要关闭浏览器,已完成更新 ' . $log . ' 条数据。';
		echo "<script>alert('".$tip."');location.href='".web_url('member/fans/initsync', array('page' => $pindex, 'acid' => $acid))."';</script>";
			exit;
...
include wl_template('member/fans');
exit();