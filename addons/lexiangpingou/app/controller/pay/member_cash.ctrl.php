...
if ($orderid) {
    $recharge = pdo_get('tg_member_recharge' , array('id' => $orderid));
} else {
    $message = "参数错误，缺少订单号.";
    die(json_encode(array('errno' => 1, 'message' => $message)));
}

if ($recharge['member_selling'] <= 0) {
...
$orderno = "CZ" . date('Ymd') . substr(time(), -5) . substr(microtime(), 2, 5) . sprintf('%02d', rand(0, 99));
$params['tid'] = $orderno;
$params['user'] = $_W['fans']['from_user'];
$params['fee'] = $recharge['member_selling'];
$params['title'] = $mname . $recharge['member_amount'] ."会员充值卡，id:" .$orderid;
...
if ($_W['isajax']) {
    ...
    $data = array(
        ...
        'tid' => $params['tid'],
        'fee' => $params['fee'],
        'card_id' => $orderid . "+".$recharge['member_amount'],
        'card_fee' => $params['fee'],
        'status' => '0',
    ...
    $message = "支付成功!";
    die(json_encode(array('errno' => 0, 'message' => $message, 'data' => $wOpt , 'orderno' => $orderno , 'id' => $orderid)));
}
?>
