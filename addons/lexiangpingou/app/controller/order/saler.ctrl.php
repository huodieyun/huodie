...
$member = pdo_fetch("SELECT * FROM " . tablename('tg_saler') . " WHERE openid = '{$openid}' and uniacid = {$uniacid} and status = 1 and merchantid = 0 ");
$record = pdo_fetchall("select * from " . tablename('tg_hexiao_record') . " WHERE openid = '{$openid}' and uniacid = {$uniacid} and status = 0 and type = 1 ");
if ($record) {
    $apply = 0.00;
...
$qbyj = 0;
$ktx = 0;
$hexiao = pdo_getall('tg_hexiao', array('uniacid' => $uniacid, 'sid' => $member['id'], 'status' => 1));
foreach ($hexiao as $item) {
    $qbyj += floatval($item['price']);
    if ($item['createtime'] < TIMESTAMP - $setting['saler_time'] * 24 * 60 * 60) {
...
if ($op == 'display') {
    ...
    $orders = pdo_fetchall("select * from " . tablename('tg_hexiao') . " where uniacid = {$uniacid} and sid = {$member['id']} and status = 1 {$com} order by id desc ");
    foreach ($orders as &$v) {
...
if ($op == 'my_cash') {
    if (checksubmit('submit')) {
        ...
        message('申请成功', app_url('order/saler', array('op' => 'withdraw', 'status' => 0)), 'success');
    }
}
...
if ($op == 'withdraw') {
    $id = $_GPC['id'];
    $status = $_GPC['status'];
    $con = " where uniacid = {$uniacid} and openid = '{$openid}' and type = 1 ";
    if (!empty($id)) {
        $con .= " and id = " . $id;
    }
    if ($status == 0) {
...
include wl_template('order/saler');
