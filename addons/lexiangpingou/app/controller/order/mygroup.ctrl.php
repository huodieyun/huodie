...
$orders = pdo_fetchall("SELECT * FROM " . tablename('tg_order') . " WHERE uniacid ='{$_W['uniacid']}' and openid='{$_W['openid']}' and is_tuan in(1,2,3) and status in(1,2,3,4,6,7,8,10) order by ptime desc");
foreach ($orders as $key => $order) {
	...
	if(empty($thistuan)){
		...
		$orders[$key]['gimg'] = $goods['share_image'];
